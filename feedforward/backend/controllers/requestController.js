      ...
      try {
        await session.withTransaction(async () => {
          ...
          const updatedFood = await Food.findOneAndUpdate(
            { _id: request.foodId, status: 'available', remainingQuantity: { $gte: request.requestedQuantity } },
            { $inc: { remainingQuantity: -request.requestedQuantity } },
            { new: true, session }
          );
          if (!updatedFood) throw new Error('Insufficient remaining quantity');

          updatedFood.status = updatedFood.remainingQuantity === 0 ? 'completed' : 'available';
...
module.exports = { createRequest, getMyRequests, getProviderRequests, updateRequestStatus };
